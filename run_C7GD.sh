...
cd VFPdata
cd GD_Hohlraum
#python ../loadPhilippegrace.py -f gdhohlraum_xmic_ne1e20cm3_interp -o ne_ -x -s -m 5e23
...
cp GD_Hohlraum/Q_gdhohlraum_xmic_10ps_IMPACTWcm2.txt flux.dat
cd ..


# Run C7.
mpirun -np $NPROC C7 -p $PROBLEM -m data/segment01.mesh -rs $RS -tf 0.0 -ok $F1ORDER -ot $F0ORDER -no-vis -fa -print -cl $CL -L $L -xp $XPOINT -minG $MING -s 3 -cfl 1e10 -S0 1.0 -dE 0.01 -Em 100 | tee C7E.out
...
# Perform analysis.
cd $DIRanalysis
python C7_analysis.py -N $NPROC -cl $CL -C7 --labelFluxExt IMPACT --pltshow -SH #-xp #--vlimshow #--AWBSstar #--AWBSoriginal
# Safe figs.
