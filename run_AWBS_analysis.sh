...
mpirun -np $NPROC C7 -p $PROBLEM -m data/segment01.mesh -rs 6 -tf 0.0 -ok 4 -ot 3 -vis -fa -print -Tmax $TMAX -Tmin $TMIN -sigma $SIGMASAFE -Tgrad $TGRAD -Z $ZBAR -ne $NE -L $L -minG $MINGSAFE
cd results/fe_analysis
python plot_C71Dprofiles.py -Z $ZBAR -n $NE -s $SIGMA
cd ../..
## Nonlocal solution very well corresponding to Pascal's solution with Aladin.
## P1 closure.
mpirun -np $NPROC C7 -p $PROBLEM -m data/segment01.mesh -rs 6 -tf 0.0 -ok 4 -ot 3 -vis -fa -print -Tmax $TMAX -Tmin $TMIN -sigma $SIGMA -Tgrad $TGRAD -Z $ZBAR -ne $NE -L $L -minG $MINGSAFE
cd results/fe_analysis
python plot_C71Dprofiles.py -Z $ZBAR -n $NE -s $SIGMA
cd ../..
## M1 closure.
mpirun -np $NPROC C7 -p $PROBLEM -m data/segment01.mesh -rs 6 -tf 0.0 -ok 4 -ot 3 -vis -fa -print -Tmax $TMAX -Tmin $TMIN -sigma $SIGMA -Tgrad $TGRAD -Z $ZBAR -ne $NE -L $L -minG $MINGSAFE -M1
cd results/fe_analysis
python plot_C71Dprofiles.py -Z $ZBAR -n $NE -s $SIGMA
cd ../..
...
mpirun -np $NPROC C7 -p 8 -m data/segment01.mesh -rs 6 -tf 0.0 -ok 2 -ot 1 -no-vis -fa -print -Tmax 800.5 -Tmin 799.5 -Tgrad 2.3 -S0 1.0 -E0 1.0 -sigma $SIGMA -Z $ZBAR -ne $NE -M1 -minG $MING
cp results/tmp/C7_1_fe_point.txt results/fe_analysis/fe_point_Ecorrect.txt

mpirun -np $NPROC C7 -p 8 -m data/segment01.mesh -rs 6 -tf 0.0 -ok 2 -ot 1 -no-vis -fa -print -Tmax 800.5 -Tmin 799.5 -Tgrad 2.3 -sigma $SIGMA -Z $ZBAR -ne $NE -minG $MING
