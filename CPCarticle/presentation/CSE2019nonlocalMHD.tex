...
\author[Release number: LLNL-PRES-768124 $\qquad\qquad$ Milan~Holec]{{\large Milan~Holec$^1$, Ben S. Southworth$^2$}}
\institute[US JAK]{
...
\date{
    ...
    \begin{center}
	\begin{tabular}{ccc} 
	\includegraphics[width=0.25\textwidth]{../figures/AMR_ALE_density_2D_nobar.png}
	&
	\includegraphics[width=0.25\textwidth]{../figures/AMR_ALE_nonlocal_Te_3D_nobar.png}
    &
    \includegraphics[width=0.32\textwidth]{../figures/C7_Calder_case5_nonlocal_kinetics_noSNB.png}
	\end{tabular}
	\end{center}
	\end{figure}
	\begin{tabular}{c}	
...
\begin{block}{AWBS electron kinetic model 7D}
\begin{eqnarray}
...
%, parallel-Approximate-Ideal-Relaxation Multigrid solver
}
%\\
...
\begin{eqnarray}
  ...
  \nonumber \\
  &=& 
  \Gamma \int_\vmag \frac{\partial f}{\partial v} \vmag^2\dI\vmag - \sigma \Te^{-0.5}
  + S_H , 
  \nonumber \\
  \vect{v}\cdot\nabla f +
  \left( \vect{E} + \vect{v}\times\vect{B}\right)\cdot\nabla_{\vect{v}} f
...
\begin{equation}
  ...
  \nonumber
\end{equation}
\begin{block}{Fixed-point iteration with preconditioner $\matr{E}(\Delta \vect{T}) = (\matr{I} - \matr{C})\cdot\matr{M}_{\sigma}(\tilde{\vect{T}} + \Delta\vect{T})^\alpha + \matr{C}\cdot\matr{D}_{\lambda}(\tilde{\vect{T}} + \Delta\vect{T})^\alpha$}
\begin{equation}
    \matr{M}_{C_V}\cdot\frac{\Delta \vect{\Te}^{k+1}}{\Delta t} + \matr{E}(\Delta \vect{\Te}^{k+1}) - \matr{E}(\Delta \vect{\Te}^{k}) = \matr{K}(f^k) - \matr{M}_{\sigma}(\tilde{\vect{\Te}} + \Delta\vect{\Te}^k)^\alpha .
  \nonumber
\end{equation}
\end{block}
Finally, we get an unconditionally stable backward Euler (SDIRK) fast iterating scheme
\begin{equation}
  \matr{M}_{C_V}\cdot\frac{\Delta \vect{\Te}^{k+1}}{\Delta t}  
  + (\matr{I} - \matr{C})\cdot\matr{M}_{\sigma}(\tilde{\vect{T}} + \Delta\vect{\Te}^{k+1})^\alpha + \matr{C}\cdot\matr{D}_{\lambda}(\tilde{\vect{\Te}} + \Delta\vect{\Te}^{k+1})^\alpha
  =  
  \matr{C}\cdot\matr{D}_{\lambda}(\tilde{\vect{\Te}} + \Delta\vect{\Te}^{k})^\alpha 
  + \matr{K}(f^k) - \matr{C}\cdot\matr{M}_{\sigma}(\tilde{\vect{\Te}} + \Delta\vect{\Te}^k)^\alpha ,  \nonumber
\end{equation}
    where the adaptive coefficient handles diffusion $\matr{C}\xrightarrow{\text{Kn}\ll1} 1$ and nonlocal transport $\matr{C}\xrightarrow{\text{Kn}>1} 0$ and $\matr{C}\in (1, 0)$ in between.
