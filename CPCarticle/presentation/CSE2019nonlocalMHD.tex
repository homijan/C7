...
\author[Release number: XXXXXXXXXXXXX $\qquad\qquad$ Milan~Holec]{{\large Milan~Holec$^0$, Ben S. Southworth$^1$}}
\institute[US JAK]{
         {\large $^0$ Lawrence Livermore National Laboratory, U.S.,
		 {\it holec1@llnl.gov}
		 }\\
         {\large $^1$ University of Colorado Boulder, U.S., 
...
{%
  ...
  \hbox{\begin{beamercolorbox}[wd=.5\paperwidth,ht=2.5ex,dp=1.125ex,leftskip=.3cm plus1fil,rightskip=.3cm]{author in head/foot}%
	...
	\includegraphics[width=0.015\textwidth]{../figures/material_77.png}\,\,\,
	\insertshortauthor
  \end{beamercolorbox}%
...
\begin{eqnarray}
  ...
  \nonumber
\end{eqnarray}
\let\thefootnote\relax\footnote{M. Holec et al, \textit{Phys. Plas.}, submitted (2019) / arXiv xxxxxxxxx.}
\end{center}
\end{frame}
...
\end{equation}
  ...
  \item AP1 (P$_N$ VEF $\xi=1/3$ strategy - CG) \it{physicists like it!}
\begin{eqnarray}
...
\end{tabular} 
\begin{equation}
  \left( \matr{M}_{_{(\tilde{\nue})}} - 
  \matr{M}_{_{(\frac{\E\cdot\vn_d}{\vmag})}} \right) \cdot 
...
AP1 formulation PCG(AMG), fixed P1 angular discretization
\begin{eqnarray}
...
\\
%Solution preconditioning (ad-hoc transition, but build as an extension 
%to the classical SH without numerical scheme restrictions) 
\let\thefootnote\relax\footnote{T. Haut et al, \textit{SIAM}, submitted (2019) / arXiv xxxxxxxxx.}
\end{frame}
