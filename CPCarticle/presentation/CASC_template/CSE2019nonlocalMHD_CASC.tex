...
\begin{eqnarray}
  ...
  \nonumber \\
  \nabla\times\vect{B} &=& \frac{4\pi}{c}
  \sigma (\E + \vect{u}_e \times \B) ,%\quad~~~ (\vect{j} \text{ depends on }f\text{ and }\vect{E}\text{ from the nonlocal model}) 
  \nonumber \\
  && \nonumber \\
  \hline \nonumber \\
  %&& \nonumber \\
...
\begin{eqnarray}
  ...
  \nonumber \\
  \nabla\times\vect{B} &=& \frac{4\pi}{c}
  \vect{j}(f) ,%\quad~~~ (\vect{j} \text{ depends on }f\text{ and }\vect{E}\text{ from the nonlocal model}) 
  \nonumber \\
  && \nonumber \\
  %&& \nonumber \\
  \text{KINETICS OF ELECTRONS} && \text{6D} \nonumber \\
...
\begin{comment} % slide4Ben
\renewcommand{\vect}[1]{\boldsymbol{#1}}
\begin{frame}
\begin{center}
\begin{myNblock}{An efficient electron kinetic model (7D) for hydrodynamics}
\begin{eqnarray}
  C_V \frac{\dI T_e}{\dI t} 
  &=& 
  - \nabla\cdot\vect{q}_e(f) 
    + \vect{j}(f)\cdot\vect{E}
    + S_{Hydro} , 
  \nonumber \\
  \vect{v}\cdot\nabla f +
  \left( \vect{E} + \vect{v}\times\vect{B}\right)\cdot\nabla_{\vect{v}} f
  &=& 
  v \tilde{\nu_e} \frac{\partial }{\partial v}\left(f - f_{MB}(T_e)\right)
    + \nu_{scat} \left(f_0 - f \right) .
  \nonumber
\end{eqnarray}
\end{myNblock}
\begin{itemize}
    \item C7 \textit{miniapp} (S$_N$ - discontinuous Galerkin FEM) 3D velocity space in spherical coordinates $(v, \phi, \theta)$
\begin{equation}
    \vect{n}\cdot\nabla f + \frac{\vect{E}\cdot\vect{n}}{v} \pdv{f}{v} 
  + \frac{E_\phi 
  - v~B_\theta}{v^2}\frac{\partial f}{\partial \phi}
  + \frac{E_\theta + v~B_\phi}
  {v^2\sin(\phi)}\frac{\partial f}{\partial \theta}
  = 
    \tilde{\nu_e} \frac{\partial }{\partial v}\left(f -f_{MB}\right)
  + \frac{\nu_{scat}}{v} \left(f_0 - f \right) .
  \nonumber
\end{equation}
\end{itemize}
\begin{tabular}{lll}
\includegraphics[width=0.3\textwidth]{AMR_ALE_density_2D_nobar.png} &
\includegraphics[width=0.3\textwidth]{AMR_ALE_nonlocal_Te_3D_nobar.png} &
\includegraphics[width=0.4\textwidth]{C7_Calder_case5_nonlocal_kinetics_noSNB.png}
\end{tabular}
\end{center}
\begin{tabular}{l}
\hline\\
