...
def SH_distribution(v, ne, Te, gradTe, Z, Gamma_ee):
    ...
    for i in range(N):
        vp = v[i]
        f1[i] = vTh(Te)**4.0 * 4.0 / Gamma_ee / ne / Zbar * D[i] * fM(ne, Te, vp) * gradTe / Te
        j1[i] = f1[i] * vp**3.0
        q1[i] = f1[i] * vp**5.0
    return f1, j1, q1
#dfdv = splev(vs, ftck, der=1)
...
corr = 2.0
## Compute AWBS following the zero current condition
## Search the right value of E field in interval (0, 10 * Ez_SH)
fAWBS1s, jAWBS1s, qAWBS1s = AWBS_distribution(vs, 0.0, ne, Te, dTedz, Zbar, 0.9999 * Ez_SH, Gamma_ee, corr)
fSH19531s, jSH19531s, qSH19531s = SH_distribution(vs, ne, Te, dTedz, Zbar, Gamma_ee)
## Fill f1 with Lorentz gas approximation as a starting value.
for i in range(0, N):
    mfpei = vs[i]**4.0 / Gamma_ee / ne / Zbar
    V2 = 0.5*vs[i]**2.0 / vTh(Te)**2.0
    #corrSH = (Zbar + 0.24) / (Zbar + 4.2)
    corrSH = SH_corr(Zbar)
    ...
    jKIPP1s[i] = fKIPP1s[i] * vs[i]**3.0
    qBGK1s[i] = fBGK1s[i] * vs[i]**5.0
    qKIPP1s[i] = fKIPP1s[i] * vs[i]**5.0
...
plt.show()
fig, ax1 = plt.subplots()
ax1.plot(vs_norm, qBGK1s, 'g-.', label=r'$q_1-$BGK')
ax1.plot(vs_norm, qAWBS1s, 'r--', label=r'$q_1-$AWBS')
ax1.plot(vs_norm, qSH19531s, 'k', label=r'$q_1-$FP')
ax1.legend(loc='upper right', fancybox=True, framealpha=0.8)
ax1.set_title('Zbar '+str(Zbar))
