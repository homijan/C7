...
\begin{eqnarray}
  ...
  \label{eq:IM0}\\
  \IM^1_{(\matr{G})} &=& \intO\matr{w} \cdot \matr{G}\cdot \matr{w}^T
  \, \dI\Omega ,
  \label{eq:IM1}\\
...
\section{Consistent E field treatment}\label{sec:Efield}
\begin{eqnarray}
...
\begin{equation}
  \IM^1_{\left( \nutot \right)}\cdot \fone   
  =
  \vmag\left(\IM^1_{(\nue)} \cdot \pdv{\fone}{\vmag}
  + \ID_{\left(\MA\right)}\right)\cdot \vfzero
  - \IM^1_{\left(\pdv{\vfzero}{\vmag} \MA\right)} \cdot
  \tE 
  - \IB_{\left( \tB \right)}\cdot \fone ,
  \nonumber
  \label{eq:semiM1hosf1}
\end{equation}
\begin{multline}
  \IM^1_{\left( \MI\right)}\cdot \int\vmag\fone\vmag^2\, \dI\vmag   
  =
  - \int\vmag\IM^1_{\left(\frac{1}{\nutot}\pdv{\vfzero}{\vmag} \MA\right)} 
  \vmag^2\, \dI\vmag \cdot\tE \\
  + \int\left[
  \vmag^2\left(\IM^1_{(\frac{\nue}{\nutot}\MI)} \cdot \pdv{\fone}{\vmag}
  + \ID_{\left(\MA\right)}\right)\cdot \vfzero 
  - \vmag\IB_{\left(\frac{1}{\nutot} \tB \right)}\cdot \fone
...
\begin{equation}
  \IM^1_{\left( \MI\right)}\cdot \vect{j}   
  =
  - \matr{A}_{\E} \cdot\tE + \vect{b}_{\E} ,
  \label{eq:GenOhmsLaw}
\end{equation}

\begin{equation}
  \intO\matr{w} \cdot \matr{w}^T\, \dI\Omega \cdot 
  \pdv{\E}{t} + 4\pi \intO\matr{w} \cdot \matr{w}^T\, \dI\Omega \cdot \vect{j} 
  = c \intO\matr{w} \cdot \matr{w}^T\, \dI\Omega \cdot \vect{rotB} ,
  \nonumber
\end{equation}
\begin{equation}
  \IM^1_{\left( \MI\right)}\cdot\pdv{\E}{t} - 4\pi\matr{A}_{\E} 
  \cdot \left(\E^n + \pdv{\E}{t} \right) = 
  - 4\pi \vect{b}_{\E}
  + c \IM^1_{\left( \MI\right)}\cdot\vect{rotB}
  \nonumber
\end{equation}
\begin{equation}
  \left[\IM^1_{\left( \MI\right)} - 4\pi\matr{A}_{\E}\right]\cdot\pdv{\E}{t} 
  = 4\pi\left(\matr{A}_{\E}\cdot \E^n - \vect{b}_{\E}\right)
  + c \IM^1_{\left( \MI\right)}\cdot\vect{rotB}
  \nonumber
\end{equation}
...
\begin{itemize}
  ...
  \item Prepare integrators on H(curl) in \eqref{eq:GenOhmsLaw}.
\end{itemize}
