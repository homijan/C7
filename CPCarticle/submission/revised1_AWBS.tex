...
\title[Modeling of nonlocal electron kinetics in low-Z plasmas relevant to inertial confinement fusion using AWBS collision operator]{\corrMH{Modeling of nonlocal electron kinetics in low-Z plasmas relevant to inertial confinement fusion using AWBS collision operator}}
...
\begin{abstract} 
\corrMH{
We study the~Albritton-Williams-Bernstein-Swartz (AWBS) collision operator 
(Phys. Rev. Lett 57, 1887 (1986)) introduced to the~nonlocal electron transport 
(D. D. Sorbo et al, Phys. Plasmas 22, 082706 (2015)) under conditions of laser-generated plasmas relevant to inertial confienement fusion, where 
the~classical hydrodynamic model fails to describe important microscopic physics related to highly mobile particles. In particular, we show some
excellent properties of the~AWBS operator for high-temperature, steep-gradient, 
low-Z plasma states, where e-e collisions play a~crucial role, by leveraging 
the~theory of local transport and simulation benchmarking against 
Vlasov-Fokker-Planck and collisional PIC codes for 
the~nonlocal transport regime.
}
\end{abstract}
...
\section{AWBS kinetic model in local diffusive regime}
...
\subsection{Summary of the~theory of local transport}
...
%\begin{eqnarray} 
%C^0_{FP}(\fzero) &=& \frac{\Gamma}{v^2}\frac{\partial}{\partial \vmag}
...
where AP1 corresponds to AWBS curve and Aladin to $C^0_{FP}$ curve. 
\corrMH{
To summarize,
...
\end{comment} % FIG3 current 

\begin{comment} % Kn variation.
...
\begin{comment} % LLNL disclaimer on the cover
...
\end{comment} % LLNL disclaimer on the cover
\end{acknowledgments}
