...
\begin{eqnarray}
  ...
  \label{eq:AWBS_f0} \\
  \pdv{f^1}{\vmag}  
  - \frac{\Zbar + \zeta_A}{\vmag\zeta_A} f^1 &=& 
  %\frac{\mfpe}{\vmag\zeta}
  ...
  %\nonumber
\end{eqnarray}
where $\zeta_A$ represents a~scaling parameter defining the~modified
...
\begin{multline}
  ...
  + \frac{\Zbar + \zeta_A}{\vmag\zeta_A}f^1_{\text{AWBS}}
  = \\
  \frac{\mfpe}{\vmag\zeta_A}\left(\frac{1}{L_{n_e}} + 
...
collision operators and a~proper evaluation of the~scaling parameter $\zeta_A$
...
\end{equation}
where 
$\xi$ the~$\Zbar$-dependence \cite{Epperlein_PoFB1991} approximation and
$\vect{q}_L$
...
\end{equation}
TODO 
In the~case of the~BGK operator 
...
frequencies as
In order to recover the~Lorentz electron-ion collision gas model 
\cite{Lorentz_1905}, we need ... 
\begin{equation}
  r(\Zbar) = \frac{\zeta_B\Zbar}{\xi(\Zbar)(\Zbar + 2 \zeta_B)}
...
The~constant $\zeta_B$ can be arbitrary in \eqref{eq:BGK_scaling} 
while there is no effect on the~local 
EDF
\begin{equation}
  \ft^1_{\text{BGK}} = - \mu 
  \left( \frac{\vmag^2}{2 \vth^2} - 4\right)\frac{\zeta_B}{r(\Zbar)}
  \frac{1}{\Zbar + 2\zeta_B}\frac{\mfpe}{L_{T_e}}\fM
  ,
  \label{eq:f1BGK_approximation}
\end{equation}
as shown in \appref{app:DiffusiveKinetics}.
$\zeta_B$ can be set for example to better address the~nonlocal transport 
behavior. \textit{$\zeta_B = 2$ gives $r(\infty) = 2$, 
...
scaling $\zeta_A$, i.e. with a~very weak dependence on $\Zbar$ as  
\begin{equation}
  \nue^* = \zeta_A(\Zbar)~\nue 
...
approaches closely the~numerical SH solution. BGK \eqref{eq:BGK_scaling} takes 
