...
\corrCPR{\st{On the~one hand} The AP1 model gives us information about 
the~electron distribution function and} 
a~macroscopic interpretation of the~microscopic EDF properties \corrCPR{is} of great
importance \corrCPR{\st{and} providing} the~bridge between kinetic and fluid description of 
...
We refer to 
\eqref{eq:NonlocalOhm} as to the~{\bf nonlocal Ohm's law}. \corrCPR{The need for a nonlocal Ohm's law to accurately capture magnetic field advection by the Nernst effect has previously been demonstrated \cite{Luciani85,Ridgers08,Brodrick18}, although a full investigation of how well our new Ohm's law captures this is beyond the scope of this article. The} proper local asymptotic to the~standard Ohm's law  
...
%CPR comment - careful here.  A non-local version of Biermann has already been derived https://journals.aps.org/prl/abstract/10.1103/PhysRevLett.88.045004 (perhaps this should be cited here?) 
...
absorption \corrCPR{(which can also distort the distribution function away from a Maxwellian \cite{Langdon80}, strongly modifying the transport \cite{Ridgers08_2}, an effect which will not be considered further here)} and 
$G = \rho C_{V_e} \nuei$ is 
...
the~time scale of the~fluid. Consequently, \corrCPR{\st{the}} Ampere's law \eqref{eq:Ampere} 
...
a~\textit{fully kinetic form of \corrCPR{\st{the}} Ampere's law} 
