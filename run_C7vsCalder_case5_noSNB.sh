...
if [ "$NAME" == "maximum" ]
then
    python C7_analysis.py -N $NPROC -s $SIGMA -cl $CL -fs 18 --labelUseC7 AP1 --labelFluxExt1 PIC-MC -SH -lD1 PIC-MC -hftit 'Heat flux Z = 1' -ktit 'Kinetics at maximum point (triangle)' --pltshow --pltTe -xp --plotmultvTh 7 -Tpts 0.058 0.046
fi
## NONLOCAL DISTRIBUTION
if [ "$NAME" == "nonlocal" ]
then
    python C7_analysis.py -N $NPROC -s $SIGMA -cl $CL -fs 18 --labelUseC7 AP1 -lF1 PIC-MC -hftit 'Heat flux Z = 1' -ktit 'Kinetics at preheat point (circle)' -lD1 PIC-MC --pltshow --pltTe -xp -SH --plotmultvTh 14 -Tpts 0.058 0.046
fi

# Safe figs.
### CASE 5 ### Z = 1 for AWBS paper.
cp heatflux.png $DIRroot/VFPdata/C7_Calder_case5_heatflux_noSNB.png
if [ "$NAME" == "maximum" ]
then
cp kinetics.png $DIRroot/VFPdata/C7_Calder_case5_kinetics_noSNB.png
fi
if [ "$NAME" == "nonlocal" ]
then
cp kinetics.png $DIRroot/VFPdata/C7_Calder_case5_nonlocal_kinetics_noSNB.png
fi
