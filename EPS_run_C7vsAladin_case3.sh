...
if [ "$NAME" == "maximum" ]
then
python C7_analysis.py -N $NPROC -s $SIGMA -cl $CL -fs 19 --labelUseC7 AWBS --labelFluxExt1 VFP -lD1 VFP -hftit 'Heat flux Z = 10' -ktit 'Kinetics at maximum point (triangle)' --pltshow --pltTe -xp -SH --plotmultvTh 7 -Tpts 0.058 0.046 --Efield --labelEfieldExt1 Aladin
fi
# NONLOCAL DISTRIBUTION
if [ "$NAME" == "nonlocal" ]
then
python C7_analysis.py -N $NPROC -s $SIGMA -cl $CL -fs 19 --labelUseC7 AWBS --labelFluxExt1 VFP -lD1 VFP -hftit 'Heat flux Z = 10' -ktit 'Kinetics at preheat point (circle)' --pltshow --pltTe -xp -SH --plotmultvTh 14 -Tpts 0.058 0.046 --Efield --labelEfieldExt1 Aladin
fi

# Safe figs.
### CASE 3 ###   Z = 10 for AWBS paper.
cp heatflux.png $DIRroot/C7_Aladin_case3_heatflux4EPS.png
cp Efield.png $DIRroot/C7_Aladin_case3_Efield4EPS.png
if [ "$NAME" == "maximum" ]
then
cp kinetics.png $DIRroot/C7_Aladin_case3_kinetics4EPS.png
fi
if [ "$NAME" == "nonlocal" ]
then
cp kinetics.png $DIRroot/C7_Aladin_case3_nonlocal_kinetics4EPS.png
fi
